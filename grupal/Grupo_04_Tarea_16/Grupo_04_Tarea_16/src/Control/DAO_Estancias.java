...
public static void ConsultarParaCombo(JComboBox control, Arbol_Estancias arbol, HashMap tabla) {
        try {
            Connection cn = ConectaDB.ObtenerConexion();
            CallableStatement cst = cn.prepareCall("{call sp_estancias_consultar_todos()}");
            ResultSet rs = cst.executeQuery();
            ...
            while (rs.next()) {
               ...
               int idHotel = rs.getInt("Hoteles_CodigoHotel");
                ...
                Estancias estan = new Estancias(codigo, pension, entrada, salida, idHotel);
                arbol.setRaiz(arbol.Insertar(arbol.getRaiz(), estan));
            }
