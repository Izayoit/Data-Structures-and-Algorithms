...
public class DAO_VuelosTuristas {
    ...
    public static void Insertar(VuelosTurista vuelo) {
        try {
            ...
            CallableStatement cst = cn.prepareCall("{call sp_vuelosturista_insertar(?,?,?,?)}");
    ...
    public static void Consultar_Todos(Arbol_VueloTurista arbol) {
        try {
            Connection cn = ConectaDB.ObtenerConexion();
            CallableStatement cst = cn.prepareCall("{call sp_vuelosturista_consultar_todos()}");
            ResultSet rs = cst.executeQuery();

            arbol.setRaiz(null); // Limpia el árbol

            while (rs.next()) {
                int codigo = rs.getInt("CodigoVuelo");
                String clase = rs.getString("Clase");
                int codViaje = rs.getInt("Vuelos_NumeroVuelos");
                int numVuelo = rs.getInt("ViajesContratados_CodigoViaje");
