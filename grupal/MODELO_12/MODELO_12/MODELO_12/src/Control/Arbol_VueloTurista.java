...
public class Arbol_VueloTurista {
    ...
    public Nodo_VuelosTurista Eliminar(Nodo_VuelosTurista Raiz, int Codigo) {
        ...
        if (Codigo < Raiz.getElemento().getCodigoVuelo()) {
            ...
            if (Raiz.getIzq() == null && Raiz.getDer() == null) return null;
            else if (Raiz.getIzq() == null) return Raiz.getDer();
            else if (Raiz.getDer() == null) return Raiz.getIzq();
            else {
    ...
    public VuelosTurista Buscar(Nodo_VuelosTurista raiz, int codigo) {
        if (raiz == null) return null;
        else if (codigo == raiz.getElemento().getCodigoVuelo()) return raiz.getElemento();
        else if (codigo < raiz.getElemento().getCodigoVuelo()) return Buscar(raiz.getIzq(), codigo);
        else return Buscar(raiz.getDer(), codigo);
    }
    ...
    public int Equilibrar(Nodo_VuelosTurista raiz) {
        ...
        if (hIzq < 0) return hIzq;
        hDer = Equilibrar(raiz.getDer());
        if (hDer < 0) return hDer;
        if (Math.abs(hIzq - hDer) > 1) return -1;
        return Math.max(hIzq, hDer) + 1;
    }

    public int ExtraerFactor_e(Nodo_VuelosTurista raiz) {
        if (raiz != null) return raiz.getFactor_e();
        return 0;
    }
    ...
    public Nodo_VuelosTurista Buscar_Minimo(Nodo_VuelosTurista raiz) {
        if (raiz == null || raiz.getIzq() == null) return raiz;
